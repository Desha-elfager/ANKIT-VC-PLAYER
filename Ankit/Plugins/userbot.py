...
from Ankit import client as ANKITUB

# To Block a PM'ed User
@ANKITUB.on_message(filters.private & filters.command("block", [".", "/"]) & filters.me & ~filters.edited)
async def ubblock(_, message: Message):
  shit_id = message.chat.id
  gonna_block_u = await message.edit_text("`Blocking User...`")
  try:
    await ANKITUB.block_user(shit_id)
    await gonna_block_u.edit("`Successfully Blocked This User`")
  except Exception as lol:
    await gonna_block_u.edit(f"`Can't Block This Guy! May be this is ramdi?` \n\n**Error:** `{lol}`")


# To Unblock User That Already Blocked
@ANKITUB.on_message(filters.command("unblock", [".", "/"]) & filters.me & ~filters.edited)
async def ubblock(_, message: Message):
  good_bro = int(message.command[1])
  gonna_unblock_u = await message.edit_text("`Unblocking User...`")
  try:
    await ANKITUB.unblock_user(good_bro)
...
@ANKITUB.on_message(filters.private & filters.command("chats", [".", "/"]) & filters.me & ~filters.edited)
async def ubgetchats(_, message: Message):
  getting_chats = await message.edit_text("`Checking Your Chats, Hang On...`")
  async for dialog in ANKITUB.iter_dialogs():
    try:
      total = await ANKITUB.get_dialogs_count()
...
# Leave From a Chat
@ANKITUB.on_message(filters.command("kickme", [".", "/"]) & filters.me & ~filters.edited)
async def ubkickme(_, message: Message):
  i_go_away = await message.edit_text("`Leaving This Chat...`")
  try:
    await ANKITUB.leave_chat(message.chat.id)
...
# Alive Message
@ANKITUB.on_message(filters.command("alive", [".", "/"]) & filters.me & ~filters.edited)
async def ubalive(_, message: Message):
  alive_msg = await message.edit_text("`Processing...`")
  alive_pic = "https://telegra.ph/file/6409c1160ffcae173900f.jpg"
  await message.reply_photo(alive_pic, caption=f"**☑️ Ankit Music Userbot is Alive 🌀** \n\n**🤖 Version** \n ↳**Bot Version:** `1.0` \n ↳**Userbot Version:** `0.0` \n\n**🐬 Info**\n ↳**Music Bot:** @{BOT_USERNAME} \n ↳**Owner:** [Click Here](tg://user?id=2127221861)")
